...
# sed_sentence_chunker3.sh

#      Created: 2017-Nov 20 | Victoria Stuart | "mail"..@t.."VictoriasJourney.com"
#         from: sed_sentence_chunker.sh
# Last updated: 2017-Nov-20

# ----------------------------------------------------------------------------
# /mnt/Vancouver/Programming/scripts/sed_sentence_chunker/sed_sentence_chunker3.sh

# For use (e.g.) in Python scripts; run this script, "sed_sentence_chunker3.sh",
# in a directory with your input (text) files in an "input/" directory; also be
# sure to make an "output/" directory.
...
# this is recursive; alt. approach is to use filename globbing: FILES=input/**/*.txt  ## = files ending in .txt, recursively through child directories
# FILES=$(find ./input -type f -iname "*.htm")              ## specific extensions, e.g. *.txt
...
for f in $FILES
    do
      ...
      sed -r 's/ pp\. ([ivx0-9])/Cho4Ph\1/g' $f > tmp_file
      ...
      sed -i -r 's/v\..?([0-9])/Eegh5eel\1/g' tmp_file
      ...
      # ----------------------------------------
      # Ellipses (ellipsis: ...), at/near the end of a line (i.e., not internally, deeper within sentences):
      ...
      # This results in (e.g.) "... the end. " The beginning of ..." (i.e. . ")
      # not being split, that we will deal with in post-processing.
      ...
      # E.g., due to the processing of abbreviations (above; restored in post-
      # processing, below) very short sentences will remain unsplit. There is
      # no facile way around this. In this regard, author "Dhont", in  the text
      # "... a theoretical model by Dhont. The resulting surface charge ..."
      # is processed as an abbreviation (like: "Molec."), and thus that sentence
      # is not split.  As authors initials, journal title abbreviations, etc.
      # may be one or more capitals, followed by lowercase letters, trying to
      # address those though post-processing rules, -- after those abbreviations
      # are restored -- will result in those abbreviations (also) being split.  :-/
      ...
      # MAIN PROCESSING LOOP -- SET 1:
      #   This breaks decimal numbers, as well as (e.g.) p.Arg62His, ...:
      #     s/([a-zA-Z0-9][\.!?]).?([A-Z0-9])/\1\n\2/g
      #   ... thus replaced with:
      #     s/([a-zA-Z][\.!?])\s([A-Z0-9])/\1\n\2/g
      sed -i -r 's/([a-zA-Z][\.!?])\s([A-Z])/\1\n\2/g
                  s/([a-zA-Z0-9][\.!?]['"'"'"]).?([A-Z0-9])/\1\n\2/g
                  s/([a-zA-Z0-9]['"'"'"][\.!?)]).?([A-Z0-9])/\1\n\2/g
                  s/([a-zA-Z0-9][\.!?][])}]).?([A-Z0-9])/\1\n\2/g
                  s/([a-zA-Z0-9][)'"]"'}][\.!?]).?([A-Z0-9])/\1\n\2/g
      ...
      # MAIN PROCESSING LOOP -- SET 2:
      sed -i -r 's/([a-zA-Z0-9][\.!?]).?(['"'"'"][A-Z0-9])/\1\n\2/g
                  s/([a-zA-Z0-9][\.!?]['"'"'"]).?(['"'"'"][A-Z0-9])/\1\n\2/g
                  s/([a-zA-Z0-9]['"'"'"][\.!?]).?(['"'"'"][A-Z0-9])/\1\n\2/g
                  s/([a-zA-Z0-9][\.!?][]})]).?(['"'"'"][A-Z0-9])/\1\n\2/g
                  s/([a-zA-Z0-9][]})][\.!?]).?(['"'"'"][A-Z0-9])/\1\n\2/g
      ...
      # ----------------------------------------
      # Delete tandem ("") quotations:

      # sed -i 's/""/"/g' tmp_file
      # sed -i "s/''/'/g" tmp_file

      # ----------------------------------------------------------------------------
      # Split otherwise normal but unsplit sentences; e.g.:
      #   ... during 7 years of follow-up in PD. Overall, 64.1% of patients ...

      # sed -i -r 's/(.[^\.]{1,5}[a-z])\.\s\s*([A-Z0-9])/\1.\n\2/g' tmp_file
      # sed -i -r 's/(.[^\.]{1,5}[a-zA-Z\ ])\.\s\s*([A-Z0-9])/\1.\n\2/g' tmp_file
      # sed -i -r 's/([[:space:]]?[a-zA-Z][^\.]{1,30})\..?([A-Z0-9])/\1.\n\2/g' tmp_file
      # sed -i -r 's/([a-z][^\.]{3,})\..?([A-Z0-9])/\1.\n\2/g' tmp_file

      # sed -i -r 's/([a-zA-Z][^\.]{2,})\.\s([A-Z0-9])/\1.\n\2/g' tmp_file

      sed -i -r 's/([a-z])\.\s([A-Z0-9])/\1.\n\2/g' tmp_file

      # NOTE: due to the processing of abbreviations (above; restored below), very short
      # sentences will remain unsplit. There is no facile way around this. In this regard,
      # author "Dhont", in "... a theoretical model by Dhont. The resulting surface charge ..."
      # is processed as an abbreviation (like: "Molec."), and thus that sentence is not split. 
      # As authors initials, journal title abbreviations, etc. may be one or more capitals,
      # followed by lowercase letters, trying to address those though post-processing rules,
      # -- after those abbreviations are restored -- will result in those abbreviations 
      # (also) being split ...  :-/
      ...
      # ----------------------------------------------------------------------------
      # split [a-z]. ['"A-Z]       ## e.g.: down."We | Atmospheric Science. "Earth's ecosystems
      # Note: need to do this near the end of this script:
      ...
      #$ f=input/chunk_test_input.txt
      #$ echo $f
      #input/chunk_test_input.txt
      # http://pubs.opengroup.org/onlinepubs/007908799/xcu/basename.html
      #$ basename $f
      #chunk_test_input.txt
